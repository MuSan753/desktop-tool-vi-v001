...
Python + PySide6。真 3D 渲染的治愈系小团子（自研软件渲染器）：白色软软的身体、蓝色渐变垂耳、头顶会发光的星芽触角、星形尾巴、大大的星空眼。无边框透明置顶，能拖、能自己溜达、会打字聊天、有情绪、记得住你说过的话。
...
**形象（v3，自研 3D 渲染器 · 小星芽）**
- 真几何 + 真光照：z-buffer 光栅化、分层着色、轮廓描边、透视相机（Qt 生态的 3D 模块在这台机器上不可用，所以自己写了渲染器）
- 参数化模型：垂耳/星空眼/触角/星星尾巴全是可动部件，支持逐顶点颜色渐变（耳朵白→蓝、眼睛蓝→紫）
...
└── cat.py                     参数化 3D 桌宠「小星芽」（Pose 驱动一切表情）
